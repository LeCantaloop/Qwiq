...
namespace Microsoft.Qwiq.Soap
{
    ...
    public class Revision : IRevision
    {
        ...
        {
            get { return _rev.Attachments.Cast<Tfs.Attachment>().Select(item => ExceptionHandlingDynamicProxyFactory.Create<IAttachment>(new Attachment(item))); }
        }
        ...
        public IDictionary<string, IField> Fields
        {
            get
            {
                return _rev.Fields.Cast<Tfs.Field>().ToDictionary(field => field.Name, field => ExceptionHandlingDynamicProxyFactory.Create<IField>(new Field(field)));
            }
        }
        ...
        {
            get { return _rev.Links.Cast<Tfs.Link>().Select(item => ExceptionHandlingDynamicProxyFactory.Create<ILink>(new Link(item))); }
        }

        /// <summary>
        /// Gets the work item that is stored in this revision.
        /// </summary>
        public IWorkItem WorkItem => ExceptionHandlingDynamicProxyFactory.Create<IWorkItem>(new WorkItem(_rev.WorkItem));
