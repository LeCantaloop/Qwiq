...
powershell -noprofile -executionpolicy bypass -command "$v = @('4.5','4.5.1','4.5.2','4.6'); gci -Filter *.csproj -Recurse -Exclude *.Tests.csproj | %% { foreach ($version in $v) { $lib = ($version.replace('.', '')); & msbuild $_ /nologo /clp:Verbosity=minimal /m /p:Configuration=Release /p:TargetFrameworkVersion=$version /p:OutputPath=bin\Release\net$lib\ /t:Rebuild } }"
IF ERRORLEVEL 1 (
  EXIT /B %ERRORLEVEL%
)
